...
// validate if caller has proper authorization
function isAuthorized(req: NextRequest): boolean {
  const authHeader = req.headers.get("authorization");
  if (!authHeader) return false;

  const token = authHeader.replace("Bearer ", "").trim();
  return token === process.env.SUPABASE_SERVICE_ROLE;
}

export async function POST(request: NextRequest) {
  console.log("POST /api/sendMessage called");
  // ✅ Check if it's a valid service role request
  const serviceRoleAuthorized = isAuthorized(request);
  const supabase = createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  // ✅ Allow if user exists OR service key is present
  if (!user && !serviceRoleAuthorized) {
