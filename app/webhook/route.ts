      ...
      if (changes[0].field === "messages") {
        ...
        if (messages) {
          ...
          for (const message of messages) {
            ...
            // Only auto-reply to user-originated messages (not system or auto-reply)
            // Optionally, check for a special flag in the message to avoid replying to auto-replies
            if (!message.from?.startsWith("auto-reply-")) {
