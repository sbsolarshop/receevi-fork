...
export async function downloadMedia(imageMessage: WebhookMessage) {
  const mediaDetails =
    imageMessage.image || imageMessage.video || imageMessage.document;
  if (!mediaDetails) {
  ...
  const firstResponse: Response = await fetch(
    `https://graph.facebook.com/v15.0/${mediaDetails.id}`,
  ...
  if (!firstResponse.ok) {
    ...
    throw new Error(
      `First response error - status: ${firstResponse.status}, response: ${responseText}`
    );
  }

  const firstResponseBody: MediaResponse = await firstResponse.json();

  const mediaResponse = await httpsGet(firstResponseBody.url, {
    headers: headerOptions,
  }); // because fetch is not working :(

  if (!mediaResponse.statusCode || mediaResponse.statusCode >= 400) {
    throw new Error(
      `Media response error - status: ${mediaResponse.statusCode}`
    );
  }
  ...
  if (mediaBody) {
    ...
    const { data, error } = await supabase.storage
      .from("media")
      .upload(
        `${imageMessage.from}/${mediaDetails.id}.${extension}`,
        mediaBody,
        {
          cacheControl: "3600",
          contentType,
